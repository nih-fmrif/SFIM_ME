...
def dep_check():
	...
	if sphinx_installed:
		...
		if StrictVersion(sphinx.__version__)<StrictVersion("1.2"):
			fails += 1
			print "++ Error: Sphinx version is too old! Please upgrade to Sphinx >=1.2.x!"
	if numpy_installed:
		print "++ INFO: Numpy version: %s" % (numpy.__version__)
		if int((numpy.__version__).split(".")[0]) <= 1 and int((numpy.__version__).split(".")[1]) <= 5:
			fails += 1
...
if __name__=='__main__':
	...
	lab.add_argument('-sort_col', dest = 'sort_col', help = 'Component feature to sort by (refers to the column in comp_table.txti. Default = 1 (most likely Kappa)', default=1)
	...
	accept, reject, middle, ignore = meica_figures.components(TED, int(args.sort_col))
