...
"""
def components(TED,sort_col):
	accept_ = np.loadtxt(TED+'/accepted.txt'     ,delimiter=',')
	reject_ = np.loadtxt(TED+'/rejected.txt'     ,delimiter=',')	
	middle_ = np.loadtxt(TED+'/midk_rejected.txt',delimiter=',')	
	ignore_ = []
	...
	for i in sorted_components:
		if i in accept_:
			accept[a,:] = ctab[np.where(ctab[:,0]==i),:]
			a = a + 1
		elif i in reject_:
			reject[b,:] = ctab[np.where(ctab[:,0]==i),:]
			b = b + 1
		elif i in middle_:
			middle[c,:] = ctab[np.where(ctab[:,0]==i),:]
			c = c + 1
		elif i in ignore_:
...
def montage(maps, accept, threshold, alpha, TED, Axial, Sagittal, Coronal, flood_num, contrast):
	...
	ctab   = np.loadtxt(TED+'/comp_table.txt')
	anat = maps[0]
	overlay = maps[1]
	...
	if anat != '' and i in accept[:,0] and Axial + Sagittal + Coronal != 0 and threshold_data != '':#if anatomcial specified and i in accept place overlay over the anatomcial
	   for i in range(overlay.shape[3]):
		...
		if Coronal:#plot coronal
			...
			plt.close()
		os.system('convert Axial_Component_' + N + '.png Sagittal_Component_' + N + '.png -pointsize 20 label:\'Component '+N+'\' -append AxSag_Component_' + N + '.png') 
		fig = plt.figure(figsize= (8,4))
		gs1 = gridspec.GridSpec(1,1)
		...
		l += 1# indecies of feats_OC2.nii differs from mefl.nii.gz this accounts for this
            ...
            pool.map(gs_montage, [ {'overlay':overlay, 'Axial':Axial, 'Sagittal':Sagittal, 'Coronal':Coronal, 'series':series, 'i':i, 'N':str(ctab[i,0].astype('int')).zfill(2),'contrast':contrast} for i in range(overlay.shape[3])]) 
            for i in range(overlay.shape[3]):
		N = str(ctab[i,0].astype('int')).zfill(2)
                FFT(TED, series, i, N)
            plt.show()
...
def gs_montage(item):
        ...
        Axial    = item['Axial']
        Sagittal = item['Sagittal']
        Coronal  = item['Coronal']
        series   = item['series']
        i        = item['i']
        N        = item['N']
...
def kappa_vs_rho_plot(accept,reject,middle,ignore,ctab,startdir,label,figures):
	try:
		...
		labels_acc = ['C{0}'.format(int(i)) for i in accept[:,0]]
		labels_rej = ['C{0}'.format(int(i)) for i in reject[:,0]]
		labels_mid = ['C{0}'.format(int(i)) for i in middle[:,0]]
		labels_ign = ['C{0}'.format(int(i)) for i in ignore[:,0]]
...
def kr_vs_component(comp_table_title):
	...
	Nc,_             = components.shape
	comp_idx_byKappa = (np.argsort(components[:,1])[::-1]).astype('int')
	print comp_idx_byKappa
	...
	kappa = plt.plot( components[comp_idx_byKappa,1])
	rho   = plt.plot( components[comp_idx_byKappa,2])
