...
"""
def TSNR_montage(item):
    k        = item['k']
    pattern  = item['pattern']
    N        = item['N']
    mean_max     = item['mean_max']
    mean_min     = item['mean_min']
    Figures_made = item['Figures_made']
    # Take the max and min TSNR values from each me-ica run
    # -----------------------------------------------------
    medn_tsnr = np.load(os.path.dirname(os.path.expanduser(pattern[k][N])) + '/axialized_nifti/medn_tsnr.npy')
    tsoc_tsnr = np.load(os.path.dirname(os.path.expanduser(pattern[k][N])) + '/axialized_nifti/tsoc_tsnr.npy')
    ...
    plt.savefig('./Report_Figures/tsnr_figure_%s.png' % int(N + Figures_made))
    plt.close()
    print("++ INFO [Figures]: Component [%s] figure completed." % int(N + Figures_made))
        


def TSNR(pattern,variables,Ncpus):
    sl=[]
...
if __name__=='__main__':
    ...
    parser.add_argument('-pattern_2', dest = 'patt2', help = 'Pathway pattern from current directory to find meica report forms', default = None)
    parser.add_argument('-plots', dest='plots', help = "This argument is rather strange in order to be more versatile.  It takes in the names of statistics and "
    ...
    parser.add_argument('-view_component', dest = 'num_comp', help = 'If given a number, will show that component from each meica report', default=False)
    parser.add_argument('-var_component', dest = 'var_comp', help = 'If specified, will show the highest variance accepted component from each report', action = 'store_true')
    parser.add_argument('-high_var', dest = 'high_var', help = 'Displays the highest variance component from each report', action = 'store_true')
    parser.add_argument('-pattern_3', dest = 'patt3', default = None, help=argparse.SUPPRESS)
    parser.add_argument('-pattern_4', dest = 'patt4', default = None, help=argparse.SUPPRESS)
    parser.add_argument('-pattern_5', dest = 'patt5', default = None, help=argparse.SUPPRESS)
    parser.add_argument('-pattern_6', dest = 'patt6', default = None, help=argparse.SUPPRESS)
    parser.add_argument('-pattern_7', dest = 'patt7', default = None, help=argparse.SUPPRESS)
    parser.add_argument('-pattern_8', dest = 'patt8', default = None, help=argparse.SUPPRESS)
    parser.add_argument('-pattern_9', dest = 'patt9', default = None, help=argparse.SUPPRESS)
    parser.add_argument('-pattern_10', dest = 'patt10', default = None, help=argparse.SUPPRESS)
    parser.add_argument('-pattern_11', dest = 'patt11', default = None, help=argparse.SUPPRESS)
    parser.add_argument('-pattern_12', dest = 'patt12', default = None, help=argparse.SUPPRESS)
    parser.add_argument('-pattern_13', dest = 'patt13', default = None, help=argparse.SUPPRESS)
    ...
    # --------------------------------------
    if options.patt1 == None:
        print("++ Error: pattern_1 not given.")
        print("++ Exiting...")
        sys.exit()

    # Use module glob to use wildcards to expand given patterns
    # ---------------------------------------------------------
    patt=[options.patt1,options.patt2,options.patt3,options.patt4,options.patt5,options.patt6,options.patt7,options.patt8,options.patt9,options.patt10,options.patt11,options.patt12,options.patt13]
    pattern=[]
    pattern_labels = []
    ...
    if os.path.isdir('%s/%s' % (options.dest,options.label)):
        subprocess.call('rm -rf %s/%s' % (options.dest,options.label),shell=True)
    subprocess.call('mkdir %s/%s' % (options.dest,options.label),shell=True)
    os.chdir(options.dest + '/' + options.label)
    ...
    num_comp=options.num_comp
    if options.num_comp != False:
    ...
    if options.var_comp != False:
        var_comp_rst(variables,num_report)
    kappa_rho_plot(variables,pattern)
    index_rst(options.num_comp,options.var_comp)
    TSNR(pattern,variables,options.Ncpus)
